$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:path+"/backend/getLevel2",//请求的url
...
$("#queryCategoryLevel2").change(function(){
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			type:"GET",//请求类型
			url:path+"/backend/getLevel3",//请求的url
...
$(".checkApp").on("click",function(){
	...
	if(status == "1" && vid != "" && vid != null){//待审核状态下才可以进行审核操作
		window.location.href=path+"/backend/check?aid="+ obj.attr("appinfoid") + "&vid="+ obj.attr("versionid");
