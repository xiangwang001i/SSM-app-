...
$(".addVersion").on("click",function(){
	var path = $("#hh6").val();
	var obj = $(this);
	window.location.href=path+"/sys/appversionadd?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href = path+"/sys/appversionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href=path+"/appinfomodify?id="+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	...
	$.ajax({
		...
		data:{"appId":appId,"info":saleSwitch},
		dataType:"json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	var path = $("#hh6").val();
	window.location.href=path+"/sys/appinfoview?appinfoid="+ obj.attr("appinfoid");
});
